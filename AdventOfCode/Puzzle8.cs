...
"bcg bc gdcbae dbca abfgde gdeba ebfacdg egcba faceg dgfbce | geadb badecg ecbfgd baecg"
        ...
        public void Run1()
        {
            ...
            foreach(var input in inputs)
            {
                ...
                string[] outputs = signals[1].Split(' ');
                count += outputs.Where(x => x.Length == 7 || x.Length == 2 || x.Length == 4 || x.Length == 3).Count();
            }
        ...
        public void Run2()
        {
            ...
            foreach (var input in inputs)
            {
                ...
                string[] signals = entries[0].Split(' ');
                string[] outputs = entries[1].Split(' ');
                int[] finalValue = new int[4];

                //char[] signal7 = signals.First(x => x.Length == 3).ToArray();
                char[] signal1 = signals.First(x => x.Length == 2).ToArray();
                char[] signal4 = signals.First(x => x.Length == 4).ToArray();
                List<char[]> signals235 = signals.Select(x=>x.ToArray()).Where(x => x.Length == 5).ToList();
                List<char[]> signals069 = signals.Select(x => x.ToArray()).Where(x => x.Length == 6).ToList();
                char[] signal3 = signals235.First(x => signal1.All(y => x.Contains(y)));
                char[] signal6 = signals069.First(x =>
                    (x.Contains(signal1[0]) && !x.Contains(signal1[1])
                    || (!x.Contains(signal1[0]) && x.Contains(signal1[1]))));
                char[] signal9 = signals069.First(x => signal4.All(y => x.Contains(y)));
                char[] signal0 = signals069.First(x => x != signal6 && x != signal9);
                char[] signal2 = signals235.First(x => x != signal3 && !x.All(y => signal9.Contains(y)));
                char[] signal5 = signals235.First(x => x != signal3 && x != signal2);

                bool IsMatch(char[] n, char[] digit)
                {
                    if (n.Length != digit.Length)
                        return false;
                    return digit.All(x => n.Contains(x));
                }
                ...
                for (int i =0; i < 4; i++)
                {
                    ...
                    {
                        char[] splitOuput = output.ToArray();
                        List<(int, char[])> tests = new List<(int, char[])> 
                        {
                            (0, signal0), (2, signal2), (3, signal3), (5, signal5), (6, signal6), (9, signal9)
                        };
                        finalValue[i] = tests.First(x => IsMatch(splitOuput, x.Item2)).Item1;
                    }
                }
